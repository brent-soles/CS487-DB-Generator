...
The reason for leaving each system in their default configuration is because of the fact most side and hobby projects, and possibly startups, use these systems in their default state rather than fine tuning it before beginning to build an applications. With our benchmark, we hope to examine which system may be better for building a performant application withouth having to adjust any settings. In addition, our benchmark aims to test the consistency of each platform by running each test in the benchmark multiple times.
